...
2. **服务发现** — systemctl 列出运行单元,支持 启动/停止/重启 按钮 + 日志查看;非 Linux 降级为进程列表
   - **应用与容器**(只读检测) — 容器(docker/podman/containerd)列表与详情(挂载/网络/环境变量/标签,敏感值脱敏)、Nginx 站点识别与健康探测、站点访问统计(TOP IP / 状态码 / 时间序列,按需读取 access.log 不轮询)
...
6. **Ansible 多机管理** — 清单/Playbook/Ad-hoc, SSE 实时流, SSH 密钥管理
7. **CI/CD 流水线** — 阶段/步骤可视化编排, 连接代码库/镜像仓库, 发布到 K8s·Docker·裸机; 手动/Webhook/cron 触发, 环节进度与实时日志(详见 [docs/cicd](./docs/cicd))
...
- 应用与容器检测需目标主机有 docker/podman/containerd 或 nginx;远程主机经 SSH 通道执行只读命令
