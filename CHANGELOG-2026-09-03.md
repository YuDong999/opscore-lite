...
## 八、v2.4: 通知渠道 + 导入导出 + 概览恢复(feat/cicd 分支)

- 通知渠道(FR-22): 钉钉(markdown+HmacSHA256 加签)/飞书(text)/企业微信(markdown)/通用 JSON; 非 2xx 记录响应体日志; 编辑器渠道下拉+钉钉密钥输入。
- 流水线导入导出(FR-23): GET /pipeline/export(凭证不导出) + POST /pipeline/import(重置 ID/凭证, 重名加后缀, 结构无效跳过, 上限 100); 列表头部导入导出按钮 —— 多机 agent 分支协作下迁移流水线配置。
