...
export default function DataGrid({ result, onEdit, connId, sql, exportSql, columnTypes, columnMeta, onFilter, onClearFilters, onSortDatabase, onAfterWrite, hidePager, emptyState, backend }: {
...
${tableFromSql} WHERE ${where}`)) return
          backend?.runWrite?.(`UPDATE ${tableFromSql} SET ${colName} = NULL WHERE ${where}`)?.then((r: any) => {
...
不可撤销。`)) return
          backend?.runWrite?.(`DELETE FROM ${tableFromSql} WHERE ${where}`)?.then((r: any) => {
              if (r?.data?.code === 'write_locked') { alert('写操作被拦截: 请先解锁写模式'); return }
  ...
  // 列头右键菜单: 排序(数据库/当前页) + 复制列名 + 列详情 + 清除排序 —— 排序入口不再依赖先选中某个单元格
  const buildColHeadMenu = useCallback((col: number): ContextMenuItem[] => {
    ...
    if (sortCol !== null) items.push({ label: '清除排序', icon: <ActionIcon kind="close" />, onClick: () => setSortCol(null) })
    return items
  }, [result, onSortDatabase, sortCol, copyCell])
          ...
          <thead>
            <tr>
              ...
              {result.columns.map((c, j) => {
                ...
                return (
                <th key={c} className={keyCls} title={keyTip + '点击排序 · 右键更多'}
                  onClick={() => {
