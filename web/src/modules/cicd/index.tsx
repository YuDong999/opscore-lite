// ── CI/CD 流水线模块(模块入口, L3 标准结构: index.tsx + shared.tsx + common.tsx + TrafficTab.tsx) ──
// 目录化样板(2026-09-19): 本目录对外只暴露这个 default export(App.tsx 懒加载 ./modules/cicd),
// 模块内文件不跨模块引用 —— 整个目录删除不影响其他模块(L3 独立性判据)。
...
import { ContextMenu, type CtxMenuItem } from '../../components/common/ContextMenu'
...
import {
  ...
  TRIGGER_TEXT, useTableSort, SortHead, statusWeight, useLocalJSON,
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_minmax(0,1fr)] gap-3 items-start lg:items-stretch lg:h-[calc(100vh-14rem)]">
        ...
        <Card className="gap-0 flex flex-col py-3 overflow-hidden lg:h-full">
          <CardHeader className="gap-0 mb-0 grid-rows-[auto] pb-1.5 shrink-0">
...
function PipelineDetail({ p, busy, tick, starred, onToggleStar, onRunDefault, onRunWithForm, onEdit, onWebhook, onCopy, onDelete, onOpenRun, onRunsChanged }: {
  ...
  return (
    <Card className="gap-0 flex flex-col py-3 overflow-hidden lg:h-full min-h-0">
      <CardHeader className="gap-0 mb-0 grid-rows-[auto] pb-2 shrink-0">
...
function PipelineEditor({ value, onClose, onSaved }: { value: Pipeline; onClose: () => void; onSaved: () => void }) {
  ...
  const [p, setP] = useState<Pipeline>(() => {
    const d = JSON.parse(JSON.stringify(value)) as Pipeline
    return { ...d, env: d.env ?? [], params: d.params ?? [], stages: d.stages ?? [] }
  })
  ...
  return (
    <Modal size="log" maxWidth={1024} style={{ width: '95vw', height: '90vh', maxHeight: '90vh' }}
      onClose={onClose} title={isNew ? '新建流水线' : `编辑流水线: ${value.name}`}>
        <div className="flex-1 overflow-y-auto hover-scroll px-6 py-4 space-y-3 min-h-0">
        <ErrBanner msg={err} onClose={() => setErr('')} />
...
function RunBranchDialog({ pipeline, onClose, onRun }: { pipeline: Pipeline; onClose: () => void; onRun: (branch: string, params: Record<string, string>) => void }) {
  ...
  return (
    <Modal maxWidth={448} onClose={() => onClose()} title={<>运行流水线: {pipeline.name}</>}>
        <ErrBanner msg={err} onClose={() => setErr('')} />
        {hasSource && (
        <div>
...
function EditPipelineDialog({ pipelineId, onClose }: { pipelineId: string; onClose: () => void }) {
  ...
  if (err) {
    return (
      <Modal maxWidth={448} onClose={() => onClose()} title="编辑流水线">
          <ErrBanner msg={err} onClose={onClose} />
          <div className="modal-actions"><Button variant="outline" onClick={onClose}>关闭</Button></div>
</Modal>
    )
  }
...
function WebhookDialog({ pipeline, onClose }: { pipeline: Pipeline; onClose: () => void }) {
  ...
  return (
    <Modal maxWidth={672} onClose={() => onClose()} title={<>Webhook 触发: {pipeline.name}</>}>
...
function RunDetail({ runId, initialStep, onStepSelect, onClose, onChanged, onRerun }: {
  ...
  return (
    <Modal size="log" maxWidth={1152} style={{ width: '95vw', height: '92vh', maxHeight: '92vh' }} onClose={onClose}
      title={<div className="flex items-start justify-between gap-3 flex-wrap w-full">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                {run.pipeline}
        ...
        <div className="flex gap-3 justify-end px-6 pb-4 pt-2 shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
...
function ScriptsTab() {
  ...
  return (
    <div className="lg:h-[calc(100vh-14rem)] lg:flex lg:flex-col">
      ...
      {editing && (
        <Modal maxWidth={672} onClose={() => setEditing(null)} title={editing.id ? `编辑脚本: ${editing.name}` : '新建脚本'}>
  ...
  return (
    <div className="lg:h-[calc(100vh-14rem)] lg:flex lg:flex-col lg:gap-3">
      ...
      <Card className="gap-0 mb-4 lg:mb-0 lg:flex-[3] lg:min-h-0 flex flex-col py-3 overflow-hidden">
        <CardHeader className="gap-0 mb-0 grid-rows-[auto] pb-3">
          <div className="flex items-center justify-between gap-2">
      ...
      {editingRepo && (
        <Modal maxWidth={576} onClose={() => setEditingRepo(null)} title={editingRepo.id ? '编辑代码仓库' : '新建代码仓库'}>
            <div className="flex gap-3">
              <div className="flex-1">
                <Label>名称 *</Label>
      ...
      {editingReg && (
        <Modal maxWidth={576} onClose={() => setEditingReg(null)} title={editingReg.id ? '编辑镜像仓库' : '新建镜像仓库'}>
            <div className="flex gap-3">
              <div className="flex-1">
                <Label>名称 *</Label>
...
function CredentialsTab() {
  ...
  return (
    <div className="lg:h-[calc(100vh-14rem)] lg:flex lg:flex-col">
      ...
      {editing && (
        <Modal maxWidth={576} onClose={() => setEditing(null)} title={editing.id ? `编辑凭据: ${editing.name}` : '新建凭据'}>
            <div className="flex gap-3 flex-wrap">
              <div className="flex-1 min-w-36">
                <Label>名称 *</Label>
