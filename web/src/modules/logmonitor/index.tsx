...
import { fmtDateTime, fmtDurCn, fmtSize } from '../../lib/format'
import { useTheme } from '../../theme'
import { useHost } from '../../components/HostContext'
import HostSelector from '../../components/HostSelector'
import EChart from '../../charts/EChart'
import { OptSelect } from '../../components/common/OptSelect'  // L2 公共层(原跨模块引用违规已清)
...
// 时间/字节格式化已上移到公共层: ../lib/format (fmtDateTime / fmtSize / fmtDurCn)
// 这里原有两个本地实现, 其中 fmtTime 是「固定格式 + 可 slice」的语义, 对应公共层 fmtDateTime。
                          ...
                          <tbody>
                            {result.items.map((e) => (
                              <tr key={e.id} className={detail?.id === e.id ? 'row-on' : ''} tabIndex={0} onClick={() => viewDetail(e.id)} onKeyDown={(ev) => ev.key === 'Enter' && viewDetail(e.id)}>
                                <td className="kib-ts">{fmtDateTime(e.ts)}</td>
                                <td>
                    ...
                    <div className="kib-drawer-body">
                      ...
                      {!detailLoading && detail.ts > 0 && (
                        <div className="kib-field-card">
                          <div className="kib-field-card-row"><span className="kib-fc-name">timestamp</span><span className="kib-fc-val">{fmtDateTime(detail.ts)}</span></div>
                          ...
                          <div className="kib-field-card-row"><span className="kib-fc-name">size</span><span className="kib-fc-val">{fmtSize(detail.size)}</span></div>
                        </div>
                      )}
        ...
        <div className="glass log-card">
            ...
            <tbody>
              {alertRules.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{fmtDurCn(r.windowMs)}</td>
                  <td>{fmtDurCn(r.cooldownMs)}</td>
          ...
          <table className="log-table">
            ...
            <tbody>
              {alertEvents.map((ev) => (
                <tr key={ev.id}>
                  <td className="log-mono">{fmtDateTime(ev.firedAt)}</td>
                  <td>{ev.ruleName}</td>
          ...
          {st && (
            <>
              <div className="log-stat-grid">
                ...
                <div className="log-stat-box">
                  <div className="log-stat-num">{fmtSize(st.totalBytes)}</div>
                  <div className="log-stat-label">索引字节</div>
                </div>
                <div className="log-stat-box">
                  <div className="log-stat-num">{st.oldest ? fmtDateTime(st.oldest) : '-'}</div>
                  <div className="log-stat-label">最早时间</div>
                </div>
                <div className="log-stat-box">
                  <div className="log-stat-num">{st.newest ? fmtDateTime(st.newest) : '-'}</div>
              ...
              <div className="log-hist-wrap">
                ...
                <div className="kib-hist-card">
                  {histOption ? (
                    <EChart option={{ ...histOption, xAxis: { ...histOption.xAxis, axisLabel: { ...histOption.xAxis.axisLabel, formatter: (v: number) => fmtDateTime(Number(v)).slice(5, 16) } } }} height={160} />
                  ) : (
                ...
                {indexes.map((ix) => {
                  ...
                  return (
                    <tr key={ix.id}>
                      ...
                      <td>{st?.bytes ? fmtSize(st.bytes) : '-'}</td>
      ...
      {tab === 'indexes' && !editing && shardCfg && (
        <div className="glass log-card" style={{ marginTop: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 12, color: 'var(--text-dim)', marginBottom: 8 }}>
            <label>分片粒度</label>
            <select className="input log-input" aria-label="分片粒度" value={shardCfg.shardBy} onChange={(e) => setShardCfg({ ...shardCfg, shardBy: e.target.value })} style={{ width: 110 }}>
              <option value="month">月</option>
              <option value="week">周</option>
              <option value="day">天</option>
            </select>
            <label>热片数</label>
            <input className="input log-input" type="number" min={0} aria-label="热分片数量" value={shardCfg.hotShards} onChange={(e) => setShardCfg({ ...shardCfg, hotShards: Number(e.target.value) })} style={{ width: 55 }} />
            <label>全局保留(天)</label>
            <input className="input log-input" type="number" min={1} aria-label="全局保留天数" value={shardCfg.defaultRetentionDays} onChange={(e) => setShardCfg({ ...shardCfg, defaultRetentionDays: Number(e.target.value) })} style={{ width: 55 }} />
