...
function HostsPanel() {
  ...
  return (
    <div>
      ...
      {groupModal && (
        <Modal onClose={() => setGroupModal(null)} showClose={false}>
      ...
      {editHost && (
        <Modal onClose={() => setEditHost(null)} showClose={false}>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:'0.5rem',marginTop:8}}>
            {templates.map(t => (
              <div key={t.id} className="card" style={{flex:'1 1 16.25rem',cursor:'pointer'}} onClick={() => createFromTemplate(t)}>
                <div style={{fontSize:'0.625rem',color:'var(--text-dim)',marginBottom:2}}>{t.category}</div>
...
function SSHPanel() {
  ...
  return (
    <div>
      ...
      {deployModal && (
        <Modal onClose={() => setDeployModal(null)} showClose={false}>
            <h3>部署公钥到主机</h3>
            <div className="form-row">
