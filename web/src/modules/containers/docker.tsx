...
function ContainersPanel({ onMsg }: { onMsg?: (m: string) => void }) {
  ...
  return (
    <>
      ...
      {confirmAct && (
        <ConfirmModal
          title={`确认执行: ${confirmAct.action === 'remove' ? '删除容器(高危)' : confirmAct.action}`}
          danger={confirmAct.action === 'remove' || confirmAct.action === 'stop'}
          busy={busy}
          okLabel={`确认${confirmAct.action}`}
          onCancel={() => setConfirmAct(null)}
          onOk={() => runAction(confirmAct.name, confirmAct.action)}
        >
      ...
      {logView && (
        <Modal size="log" maxWidth={860} onClose={() => setLogView(null)}
          title={<>日志: {logView.name} <span className="pill pill-sub">@ {logView.target} · 最近300行</span></>}>
      ...
      {statsView && (
        <Modal className="stats-modal" onClose={() => setStatsView(null)}
          title={
            <>
              监控: {statsView.name}
...
function ContainerRunModal({ initial, host, onClose, onDone }: {
  ...
  return (
    <Modal size="log" maxWidth={720} onClose={onClose}
      title={<>
...
function ImagesPanel({ onMsg }: { onMsg?: (m: string) => void }) {
  ...
  return (
    <>
          ...
          <div style={{ marginTop: '0.625rem' }}>
            ...
            <div className={`pull-progress ${!job.done && job.layersTotal === 0 ? 'indeterminate' : ''}`}>
              <div className="pull-progress-bar" style={{
                ...
                background: job.err ? 'var(--danger)' : undefined,
              }} />
            </div>
            {job.lines.length > 0 && (
      ...
      {migrateModal && (
        <Modal maxWidth={540} onClose={() => setMigrateModal('')} showClose={false}>
      ...
      {tagModal && (
        <Modal maxWidth={560} onClose={() => setTagModal(null)} showClose={false}>
      ...
      {histModal && (
        <Modal size="log" maxWidth={860} onClose={() => setHistModal(null)} title={<>镜像历史层: {histModal}</>}>
...
function VolumesPanel({ onMsg }: { onMsg?: (m: string) => void }) {
  ...
  return (
    <>
      ...
      {confirmDel && (
        <ConfirmModal
          title={`删除卷: ${confirmDel}`}
          danger
          okLabel="确认删除"
          onCancel={() => setConfirmDel(null)}
          onOk={() => doRemove(confirmDel)}
        >
...
function F({ label, children, hint, required, wide }: { label: string; children: any; hint?: string; required?: boolean; wide?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: '0.75rem', color: 'var(--text-dim)', minWidth: 0, ...(wide ? { gridColumn: '1 / -1' } : {}) }}>
      <span>{label}{required && <span style={{ color: 'var(--danger)', marginLeft: 2 }}>*</span>}{hint && <i style={{ fontStyle: 'normal', opacity: 0.7 }}> · {hint}</i>}</span>
      {children}
    </div>
  )
}
                ...
                <Section title="HEALTHCHECK (健康检查)">
                  {!m.healthcheck ? (
                    ...
                    <>
                      ...
                      <button type="button" className="btn-glass-soft btn-glass-soft-sm" style={{ color: 'var(--danger)' }} onClick={() => set({ healthcheck: null })}>移除健康检查</button>
                    </>
                  )}
                </Section>
              </div>
    ...
    <Card title="Compose 项目编排" subtitle="配置保存在目标主机 /tmp/.opscore-compose/<project>/ · 后续操作无需重复粘贴">
      <div className="btn-row" style={{ alignItems: 'center', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8125rem' }}>
          ...
          {projHint && <span style={{ color: 'var(--danger)', fontSize: '0.6875rem' }}>{projHint}</span>}
        </label>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {m.services.map((svc, i) => (
                  <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button type="button" className="btn-glass-soft btn-glass-soft-sm" style={{ color: 'var(--danger)' }} onClick={() => removeService(i)} title="删除服务">✕</button>
                  </div>
                ))}
...
function ExecModal({ name, host, onClose }: { name: string; host: string; onClose: () => void }) {
  ...
  return (
    <Modal size="log" maxWidth={780} onClose={onClose}
      title={<>容器内执行: <span className="mono">{name}</span> <span className="pill pill-sub">docker exec · sh -c</span></>}>
...
function ToolOutModal({ title, body, onClose }: { title: string; body: string; onClose: () => void }) {
  return (
    <Modal size="log" maxWidth={760} onClose={onClose} title={title}>
      <pre className="code-block" style={{ margin: 0, maxHeight: 480, overflow: 'auto', fontSize: '0.6875rem', whiteSpace: 'pre-wrap' }}>{body}</pre>
    </Modal>
  )
}

// 二次确认弹窗已上移到公共层: src/components/common/Modal.tsx (ConfirmModal)
// 本模块原有的局部实现与此前 3 处展开写法均已替换为公共版。
...
function EnvEditModal({ host, name, onClose, onMsg }: { host: string; name: string; onClose: () => void; onMsg?: (m: string) => void }) {
  ...
  return (
    <Modal maxWidth={560} onClose={onClose} showClose={false}>
      <h3>修改环境变量 — {name}</h3>
...
function DockerCpModal({ host, name, onClose, onMsg }: { host: string; name: string; onClose: () => void; onMsg?: (m: string) => void }) {
  ...
  return (
    <Modal maxWidth={620} onClose={onClose} showClose={false}>
      <h3>文件互拷 — {name}</h3>
...
function NetConnectModal({ host, net, action, containers, onClose, onMsg }: {
  ...
  return (
    <Modal maxWidth={520} onClose={onClose} showClose={false}>
      <h3>{titleText}</h3>
      <div style={{ marginTop: '0.75rem', display: 'grid', gap: '0.5rem' }}>
...
function ImageLoadModal({ host, onClose, onMsg }: { host: string; onClose: () => void; onMsg?: (m: string) => void }) {
  ...
  return (
    <Modal maxWidth={540} onClose={onClose} showClose={false}>
