...
export default function K8sCertsModal({ cluster, clusterName, onClose, onMsg }: K8sCertsModalProps) {
  ...
  return (
    <Modal maxWidth={900} onClose={() => { if (!busy && !planBusy) onClose() }} closeOnOverlay={!busy && !planBusy}
      style={{ width: '94vw', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}
      title={<>证书管理 — {clusterName}</>}
      headRight={
