...
import K8sActionPanel from './K8sActionPanel'
import type { ActionSpec } from './K8sActionPanel'
import ExecTerminalModal from './ExecTerminalModal'
              ...
              <table className="data-table">
                ...
                <tbody>
                  ...
                  {sortedRows.map((r, i) => {
                    ...
                    return (
                    <tr key={rk} className={selected.has(rk) ? 'row-selected' : ''} style={{ cursor: 'pointer', background: warnRow ? 'color-mix(in srgb, var(--danger) 8%, transparent)' : undefined }} onDoubleClick={() => dblRow(r)}
...
<td className="row-ops" style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
      ...
      {actionPanel && clusterID && (
        <Modal maxWidth={720} style={{ width: '90vw' }} onClose={() => setActionPanel(null)} showClose={false}>
...
function K8sOverview({ clusterID, clusterName }: { clusterID: string; clusterName: string }) {
  ...
  const loadAll = () => {
    ...
    getJSON<{ ok: boolean; nodes: any[]; degraded?: boolean; reason?: string }>(`/api/plugins/containers/k8s/metrics/nodes?cluster=${clusterID}&_=${Date.now()}`)
      .then((d) => { if (d.ok) { setNodeMetrics(d.nodes || []); setMetricsDegraded(d.degraded ? { degraded: true, reason: d.reason || '' } : null) } })
      .catch(() => {})
  ...
  useEffect(() => {
    const t = setInterval(() => {
      getJSON<{ ok: boolean; nodes: any[]; degraded?: boolean; reason?: string }>(`/api/plugins/containers/k8s/metrics/nodes?cluster=${clusterID}&_=${Date.now()}`)
        .then((d) => { if (d.ok) { setNodeMetrics(d.nodes || []); setMetricsDegraded(d.degraded ? { degraded: true, reason: d.reason || '' } : null) } }).catch(() => {})
      getJSON<{ ok: boolean; pods: any[] }>(`/api/plugins/containers/k8s/metrics/pods?cluster=${clusterID}&ns=all&top=500&_=${Date.now()}`)
        .then((d) => d.ok && setTopPods(d.pods || [])).catch(() => {})
    }, 30000)
    return () => clearInterval(t)
  ...
  return (
    <div className="k8s-overview">
      ...
      {/* metrics-server 降级黄条: 兜底链路(kubelet 直读)在出数, 但 kubectl top/HPA 不可用需提示 */}
      ...
      <div className="grid grid-3">
        ...
        <GaugeCard title="节点 Ready 率" value={ov.nodesTotal ? (num(ov.nodesReady) / num(ov.nodesTotal)) * 100 : 0}
          color={accent} height={150}
          footer={<div className="stat-row"><span>{num(ov.nodesReady)}</span><span className="dim">/ {num(ov.nodesTotal)} 台</span></div>} />
        <Card title="告警事件">
...
function RegisterModal({ onClose, onDone }: { onClose: () => void; onDone: (ok: boolean, msg: string) => void }) {
  ...
  const loadDefault = async () => {
    ...
    try {
      ...
      if (!d || !d.found) {
        // found:false 现在带 reason+probed(探测过的本机路径清单)。关键消歧:「本机」= 运行本系统的机器,
        // K8s 在远程主机时这里永远找不到 —— 必须引导走「从该主机拉取」, 否则就是无声死胡同。
        const probed = Array.isArray(d?.probed) && d.probed.length ? `（已探测: ${d.probed.join('、')}）` : ''
        setScanInfo(
          '✗ 本机（运行本系统的机器）未找到默认 kubeconfig' + probed +
          ' —— K8s 在远程主机时请改用「从该主机拉取」；确在本机可设 OPSCORE_KUBECONFIG 环境变量，或直接粘贴/上传'
        )
        return
      }
      ...
      setScanInfo(`✓ 已读取本机 ${d.path}${extra}`)
    } catch (e) {
  ...
  const submit = (overwrite = false) => {
    ...
    setBusy(true)
    postJSON('/api/plugins/containers/k8s/clusters', { name: name.trim(), kubeconfig, overwrite })
      .then((d: any) => {
        if (d.ok) {
          const c = d.cluster || {}
          const how = overwrite ? '凭据已覆盖更新' : '已注册'
          onDone(true, `✓ 集群 ${c.name || name} ${how}${c.status === 'ready' ? ` · 连接正常 (${c.version})` : ' · ⚠ 已注册但暂不可达'}`)
        } else if (d.exists) {
          // 同名集群(常见: K8s 重装后旧凭据连不上, 重新拉了新 kubeconfig) → 提供覆盖更新的兜底
          setBusy(false)
          if (window.confirm(`集群「${name.trim()}」已存在。是否用当前填入的 kubeconfig 覆盖更新其凭据？（原集群的其它注册信息保留）`)) {
            submit(true)
          }
          return
        } else {
  ...
  return (
    <Modal size="log" maxWidth={640} onClose={onClose} title="注册集群">
          ...
          <label style={{ fontSize: '0.8125rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '4px 0', flexWrap: 'wrap' }}>
              <button className="btn-glass-soft" onClick={loadDefault} disabled={scanBusy}
                title="读取本机(运行本系统)的默认 kubeconfig；K8s 部署在远程主机时请用「从该主机拉取」">
                {scanBusy ? '读取中…' : '读取本机默认配置'}
              </button>
            ...
            {scanInfo && (
              <div style={{ fontSize: '0.75rem', marginBottom: 4, lineHeight: 1.5,
                color: scanInfo.startsWith('✓') ? 'var(--ok)' : scanInfo.startsWith('✗') ? 'var(--danger)' : 'var(--text-dim)',
                whiteSpace: 'pre-wrap' }}>
                {scanInfo}
              </div>
            )}
            <textarea className="input" value={kubeconfig} onChange={(e) => setKubeconfig(e.target.value)}
              placeholder="点上方按钮自动加载（本机默认配置 / 从主机组主机拉取）；或粘贴 kubeconfig YAML（凭据仅落盘服务端 data/kubeconfigs/，权限 0600）"
...
function ResourceModal({ info, onClose, act, onMsg }: {
  ...
  return (
    <>
    <Modal size="log" onClose={onClose}
      maxWidth={info.kind === 'events' ? 1100 : 880}
      style={info.kind === 'events' ? { width: 'min(96vw, 1100px)' } : undefined}
      title={<>
                ...
                <>
                  ...
                  <div className="table-wrap"><table className="data-table">
                    ...
                    <tbody>{detail.events.map((e: any, i: number) => (
                      <tr key={i} style={e.type === 'Warning' ? { background: 'color-mix(in srgb, var(--danger) 8%, transparent)' } : undefined}>
              ...
              <div className="table-wrap">
                <table className="data-table">
                  ...
                  <tbody>
                    ...
                    {evRows.map((e, i) => (
                      <tr key={i} style={{ background: e.type === 'Warning' ? 'color-mix(in srgb, var(--danger) 8%, transparent)' : undefined }}>
      ...
      {/* 日志(静态, 近200行)弹窗 */}
      {logModalOpen && (
        <Modal maxWidth={800} onClose={() => setLogModalOpen(false)} showClose={false}
          style={{ width: '90vw', maxHeight: '70vh', display: 'flex', flexDirection: 'column' }}>
...
function NodeJoinModal({ cluster, onClose, onMsg }: { cluster: string; onClose: () => void; onMsg: (m: string) => void }) {
  ...
  return (
    <Modal maxWidth={640} style={{ width: '90vw' }} onClose={onClose} showClose={false}>
...
function HelmPanel({ clusterID, onMsg }: { clusterID: string; onMsg: (m: string) => void }) {
  ...
  return (
    <div className="card k8s-table-card">
      ...
      {detail && (
        <Modal size="log" maxWidth={760} onClose={() => setDetail(null)}
          title={<>Release: <span className="mono">{detail.name}</span></>}
          headRight={
            <div style={{ display: 'flex', gap: 4 }}>
...
function HelmInstallModal({ clusterID, onClose, onDone }: {
  ...
  return (
    <Modal maxWidth={640} onClose={onClose} title="安装/升级 Helm Release">
