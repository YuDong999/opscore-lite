...
export default function ConnectionPanel({
  ...
  // Esc 与「返回」同语义: 选引擎步 = 取消向导; 填配置步 = 返回上一步(与头部返回键一致)
  const backFromConfig = () => setStep(editing?.id ? 'list' : 'pick-engine')
  useEffect(() => {
    if (step === 'list') return
    const onKey = (e: KeyboardEvent) => {
      if (e.key !== 'Escape') return
      if (step === 'pick-engine') cancelWizard()
      else if (step === 'fill-config') backFromConfig()
    }
  ...
  if (step === 'pick-engine') {
    ...
    return createPortal(
      <div className="db-wizard-backdrop" onClick={cancelWizard}>
      <div className="db-conn-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="db-conn-panel-body">
  ...
  if (step === 'fill-config' && editing && pickedEngine) {
    ...
    return createPortal(
      <div className="db-wizard-backdrop" onClick={backFromConfig}>
      <div className="db-conn-panel" onClick={e => e.stopPropagation()}>
        <div className="db-conn-list-head">
