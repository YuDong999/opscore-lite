    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, height: '100%', minHeight: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'nowrap' }}>
        ...
        <span className="dim" style={{ fontSize: '0.72rem' }}>
          {rows.length} 表 · 约 {fmtRows(totals.rows)} 行 · 数据 {fmtSize(totals.data)} · 索引 {fmtSize(totals.index)}
        </span>
          ...
          {sorted.map(r => (
            <div key={r.name} className="db-drv-card" style={{ cursor: onOpenTable ? 'pointer' : 'default' }} onClick={() => onOpenTable?.(r.name)}
              ...
              <div className="db-drv-card-head">
                <span className="db-engine-dot" style={{ background: 'var(--ok)' }} />
              ...
              <div className="db-drv-meta dim">
                <span>数据 {fmtSize(r.dataSize)}</span>
                <span>·</span>
                <span>索引 {fmtSize(r.indexSize)}</span>
                {r.engine && <span>·</span>}
            ...
            <tbody>
              {sorted.map(r => (
                <tr key={r.name} style={{ cursor: onOpenTable ? 'pointer' : 'default' }} onClick={() => onOpenTable?.(r.name)}>
                  ...
                  <td>{fmtSize(r.dataSize)}</td>
                  <td>{fmtSize(r.indexSize)}</td>
                  <td>{r.engine || '-'}</td>
