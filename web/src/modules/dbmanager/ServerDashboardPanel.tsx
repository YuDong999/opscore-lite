...
import { fmtByteRate, fmtNum, fmtPerSec, fmtSize } from '../../lib/format'
...
// 保留本文件私有的 fmtUptime：输出是「2天 5时 30分」这种中文长格式，
// 与 MultiOverview/ResourcesModule 的「2d 5h」紧凑格式不是一回事，不强行统一。
  ...
  const cards = useMemo(() => {
    ...
    if (isMysql) {
      ...
      return [
        ...
        { label: 'QPS', value: qps != null ? fmtPerSec(qps) : '采样中…' },
        { label: 'TPS', value: (() => { const a = rate('Com_commit'), b = rate('Com_rollback'); return a != null && b != null ? fmtPerSec(a + b) : '采样中…' })() },
        { label: '入流量', value: rate('Bytes_received') != null ? fmtByteRate(Math.round(rate('Bytes_received')!)) : '采样中…' },
        { label: '出流量', value: rate('Bytes_sent') != null ? fmtByteRate(Math.round(rate('Bytes_sent')!)) : '采样中…' },
    ...
    if (isPg) {
      const tps = (() => { const a = rate('xact_commit'), b = rate('xact_rollback'); return a != null && b != null ? fmtPerSec(a + b) : '采样中…' })()
      ...
      return [
        ...
        { label: 'WAL 生成速率', value: rate('wal_bytes') != null ? fmtByteRate(Math.round(rate('wal_bytes')!)) : '—' },
        ...
        <div className="db-dash-status">
          ...
          <div className="table-wrap" style={{ maxHeight: '18rem' }}>
            <table className="db-table">
              ...
              <tbody>
                {statusRows.map(([k, v]) => (
                  <tr key={k}><td><code>{k}</code></td><td>{fmtSize(Number(v) || 0) !== '0 B' && /^Bytes|^Data_|^Innodb_data|^Innodb_written/.test(k) ? `${v} (${fmtSize(Number(v))})` : v}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
