...
type SyncRequest struct {
	...
	TargetSchema      string      `json:"targetSchema,omitempty"` // 目标模式(PG 族三级命名时必填; 空=回落 TargetDB)
...
// EffectiveSchema 目标端模式限定: 显式 TargetSchema 优先;
// 未指定时 —— MySQL 族库即模式, 回落 TargetDB; PG 族不限定(按连接 search_path 落到默认模式)。
func EffectiveSchema(req SyncRequest, dstD Dialect) string {
	if req.TargetSchema != "" {
		return req.TargetSchema
	}
